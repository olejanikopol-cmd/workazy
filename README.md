...
## Нативное приложение для iPhone (`mobile/`)

Мобильный клиент запускается отдельно от веб-приложения. Нужен Node.js, совместимый
с установленным Expo SDK 57 (в проверках использован Node 24), npm и для симулятора
macOS с полным Xcode, выбранным через `xcode-select`, и установленным iOS Simulator.

```bash
cd mobile
npm ci
npm start
# Нажмите i для установленного симулятора или откройте QR на устройстве
npm run ios
```

Expo Go должен поддерживать SDK проекта; он удобен для разработки, но не заменяет
проверку разрешений, локальных уведомлений и записи медиа в собственной сборке.
Для локальной нативной сборки (генерирует `ios/`, требует Xcode и CocoaPods):

```bash
cd mobile # если вы в корне репозитория
npx expo run:ios
# Физический iPhone: подключение/доверие, Developer Mode и Apple signing
npx expo run:ios --device
```

Для устройства/релиза нужно настроить bundle identifier и подпись Apple.
EAS/dev-client не настроены; не предполагается наличие готовой dev build или
Apple-аккаунта. `npm run export:ios` проверяет JS bundle, но не создаёт подписанный
архив и не доказывает работу на iPhone.

Мобильные планы, календарь, дневник, идеи и финансы хранятся локально в AsyncStorage;
аудио/видео — в файлах приложения. Веб-синхронизация, загрузка медиа и расшифровка
в мобильный runtime не подключены. Веб-функции ниже описывают отдельный веб-клиент.
Удаление приложения может удалить локальные данные; встроенного резервного
копирования/экспорта мобильных данных пока нет. Onboarding имеет отдельный ключ;
его повтор из настроек не сбрасывает записи.

Камера и микрофон запрашиваются при записи, уведомления — по явному нажатию.
Calendar и Finance используют локальные уведомления с общим бюджетом 48 запросов,
включая чужие/неизвестные. Доставка зависит от системных настроек; название записи
может быть видно на экране блокировки. Для мобильного клиента не нужны Telegram,
серверные ключи, авторизация или настройка существующего веб-бэкенда.

Проверки из `mobile/`: `npm test`, `npm run typecheck`, `npm run lint`,
`npx eslint .`, `npx expo install --check`, `npx expo-doctor`, `npm run export:ios`.
Из корня: `npm run build` и `git diff --check`.

**Код Slice 7 и наблюдение на устройстве — разные статусы.** Реальная приёмка iPhone
остаётся PENDING. Отдельные «Задания» исключены из нативного продукта по продуктовому
решению; «Цели» реализованы внутри «Планов» для недели, месяца и года;
polish не подменяет их реализацию. Подробности: [Migration status](mobile/MIGRATION_STATUS.md),
[точный чек-лист iPhone и App Store](mobile/NATIVE_ACCEPTANCE.md).
