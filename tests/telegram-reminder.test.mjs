...
test("GitHub keeps an authenticated exact-time reminder poller alive", async () => {
  ...
  assert.match(source, /cron: "40 \* \* \* \*"/, "резервный воркер стартует заранее до отправки в :55");
  ...
  assert.match(source, /clock\.minute === "55"/);
  assert.match(source, /timeZone: "Europe\/Kyiv"/);
  assert.match(source, /Hourly digest checked at Kyiv :55/);
  assert.doesNotMatch(source, /consecutiveFailures >= 5\) throw/);
  assert.doesNotMatch(source, /cron: "(?:0|53) \* \* \* \*"/);
